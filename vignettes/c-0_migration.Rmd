...
```r
migration(path = NULL, tz, sections, success.arrays = NULL, max.interval = 60, minimum.detections = 2, 
  ...
  if.last.skip.section = TRUE, replicates = NULL, disregard.parallels = TRUE)
```
...
This argument controls wether or not the presence of parallel arrays (in multi-branch analyses) should potentially lead to the invalidation of efficiency peers. This argument is a bit tough to explain in few words, so I recommend that you [have a look at how efficiency is calculated in migration()](c-4_migration_efficiency.html#cjs-and-multiple-branches).
