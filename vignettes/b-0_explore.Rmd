...
explore(path = NULL, tz, max.interval = 60, minimum.detections = 2, start.time = NULL, stop.time = NULL,
  ...
  override = NULL, report = TRUE, GUI = c("needed", "always", "never"))
```
...
The `report` option activates a series of print functions that draw tables and figures for later integration in an html report. When you activate this option, a new *Report* subdirectory is created, where all the relevant files are stored. Before finishing, actel assembles the [html report](b-2_explore_results.html#actel_explore_report.html-if-report-true) and stores it in your working directory. If you already have a report in your working directory, actel will create a new file with a different name.
...
Note:
  : Even if something fails during the report printing, your results will still be saved the *actel_explore_results.RData* file in your working directory!
