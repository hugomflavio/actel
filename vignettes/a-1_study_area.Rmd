...
In the migration() and residency() analyses, the sections inform actel of the study area parts for which you would like to get specific metrics.

When you type the sections into the `migration()` function, you should ordered them in the order that you expect your fish will cross the sections. For example, if you expect your fish to move from the river to a fjord and ultimately to the sea (and you have hydrophone  arrays in all these sections), then `sections = c("River", "Fjord", "Sea")`. In the other hand, if you expect your fish to migrate upstream, then `sections = c("Sea", "Fjord", "River")`.
