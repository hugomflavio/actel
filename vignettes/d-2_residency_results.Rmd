...
The `time.ratios` contains detailed information on the time spent at each location, per day (or hour, if `timestep = "hours")` , for each of your fish. These tables can be quite long. Here is an example:
...
The `time.locations` is a data frame showing the place where each fish spent the most time during each day. It is a crucial middle step between the `time.ratios` and the `global.ratios`.
...
1. The absolute number of fish at each location in each day/hour:
...
2. The percentage of fish at each location in each day/hour:
