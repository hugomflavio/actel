...
Array __D__ has three arrays directly or indirectly after it: __E__, __F__ and __G__. When `disregard.parallels = TRUE`, actel will ask the following question, for each of the arrays coming after __D__:
    ...
    Can a fish get to array [x] without having to pass through array D?

This assumes that the fish is expected to move only in one direction (i.e. from release to a success array). If the answer to this question is no, then array [x] is a valid efficiency peer of array __D__. Both arrays __E__ and __F__ fulfil this requirement, as the only array coming before them is __D__ itself. However, array __G__ fails this requirement, because fish can get to array __G__ coming from array __C__, which is not connected to array __D__.
...
In this case, since `disregard.parallels = FALSE`, actel asks an additional question:
    ...
    1) Can a fish get to array [x] without having to pass through array D?
...
Regardless of the mechanics explained above, arrays __C__, __E__, __F__ and __G__ would never have any valid efficiency peers. As such, inter-array efficiency cannot be calculated for them using a CJS model. However, you have the chance to calculate intra-array efficiency, if an array is [composed by two lines of receivers](c-0_migration.html#replicates). You can find the efficiency peers of each array in the [`arrays` object](c-2_migration_results.html#arrays).
