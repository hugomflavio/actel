...
```r
transitionLayer(shape, size, EPSGcode, coord.x, coord.y, buffer, directions)
```
...
1. The `coord.x` and `coord.y` allow actel to search your spatial.csv file and match the station positions with the extent of the shapefile. Write the names of the columns containing the x and y coordinates in these two arguments.
...
```r
distancesMatrix(t.layer, coord.x, coord.y, EPSGcode)
```

The `t.layer` is the transition layer you just calculated. Fill in the column names of the X and Y coordinates of your 'spatial.csv' file in `coord.x` and `coord.y`. Like before, the `EPSGcode` is the code of the coordinate system of the points (the same as the shapefile's). When you run this command, actel will ask you if you want to save the output to a 'distances.csv' file. Alternatively, you can check the output within R and then save the matrix yourself!
...
```r
# Start by creating a transition layer:
x <- transitionLayer(shape = "my_shapefile.shp", size = 20, EPSGcode = 32632)

# You can opt to save the transition layer for later use by running
# save(x, file = "transition.layer.RData")
# You can then load it by running
# load("transition.layer.RData")
# This will restore the object "x" to your R session

dist.mat <- distancesMatrix(t.layer = x, coord.x = "x", coord.y = "y", EPSGcode = 32632)
# Reply "y" when prompted to save the distances matrix directly to a 'distances.csv' file
# or, alternatively, inspect the output and save it manually.
