---
title: "7.0) Errors and messages"
author: "Hugo Flávio"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{7.0) Errors and messages}
...
1. [__Errors and messages__](g-0_messages.html)
...
Note :
  ...
  : As actel evolves, some new errors and messages may accidentally be left out of this page. If you need help with a message, warning or error that is not reported here, please open an issue at [actel's GitHub repository.](https://github.com/hugomflavio/actel/issues/new)
...
````
Warning: At least one release site has been indicated in the spatial.csv file, but no release sites were specified in the biometrics.csv file.
...
````
Warning: Release sites were not specified in the spatial.csv file but more than one release site is reported in the biometric data.
...
````
Error: The following release sites were listed in the biometrics.csv file but are not part of the release sites listed in the spatial.csv file: [...]
...
````
Error: Not all the expected first arrays of the release sites exist.\nUnknown expected first arrays: [...].
...
````
Warning: No arrays were found that match section(s) [...]. There could be a typing mistake!
...
```
Warning: File [...] does not match to any of the supported hydrophone file formats!
...
Warning: One file was excluded from further analyses.

Warning: # files were excluded from further analyses.
```
...
-->
````
Warning: Replacing spaces in array names to prevent function failure.
````
...
Warning: Some fish contain no release site information. You may want to double-check the data.
   Filling the blanks with 'unspecified'.

M: No 'Group' column found in the biometrics.csv file. Assigning all fish to group 'All'.

Warning: Some fish contain no group information. You may want to double-check the data.
   Filling the blanks with 'unspecified'.
```
...
-->
```
Warning: Detections from receivers [...] are present in the data, but these receivers are not part of the study's stations. Double-check potential errors.
```
...
```
Warning: Fish [...] was detected in one or more receivers that are not listed in the study area (receiver(s): [...])!
Possible options:
...
Which option should be followed?(a/b/c)
````

It would appear that some detections for a receiver were registered when the receiver was not deployed. If those detections are real, you should edit the deployment times for the receiver. Otherwise, if those are erroneous detections (e.g. recorded during transport or tag activation), then you can simply discard them. If you already know that these exceptions are not a problem, you can select option "c", or run the the analysis with `discard.orphans = TRUE`.

----------------------

```
Warning: No detections were found for receiver(s) [...].
```
...
<!-- 1. **Impassable movement**
 -->
````
Warning: Fish [...] made an impassable jump: It is not possible to go from array [...] to [...].
...
-->
````
Warning: Fish [...] jumped through [#] arrays in events [#] -> [#] ([Array] -> [Array])
...
-->
````
Warning: Fish [...] had an average speed of [#] m/s from valid event [#] to [#] ([Array] -> [Array])
M: Opening valid movements table of fish [...] for inspection:
[...]
Would you like to render any movement event invalid?(y/N/comment)
````
...
````
Warning: Fish [...] was detected [#] times at three or less stations of array [...] over [#] days and then disappeared. Could it be inactive?

Warning: Fish [...] was detected [#] times at stations less than 1.5 km apart in array [...] over [#] days and then disappeared. Could it be inactive?
...
If you included any transmitters in the `override` argument, this message will show up when it is their turn to be analysed. You can find more details on [how to operate full manual mode here.](e-0_manual_mode.html)
...
```
Warning: Fish [...] was detected in an array that is not after its release site! Opening relevant data for inspection.
   Release site: [...]
...
-->
```
Warning: Inter-section backwards movements were detected for Fish [...].

Warning: Inter-section backwards movements were detected for Fish [...] and the last events are not ordered!
```

A fish moving a bit up and down is not much to worry about. But if these backward movements involve moving from one section of your study area to another (i.e. a fish migrating downstream moved to the sea and then back to the river), actel will issue a warning. You must then verify the movements and take action if desired.

You can [read more about backward movements here.](c-1_migration_processes.html#backwards-movements)

----------------------

### Exceptions exclusive to the residency function


````
Warning: Section movements with less than [#] detections are present for fish [...].
````

This exception is controlled by the `section.minimum` argument. If a fish has a number of consecutive detections that is lower than this threshold, you are given a chance to inspect the data and render movement events invalid, if necessary.
...
````
Warning: No fish passed through array [...].

Warning: No fish passed through any of the efficiency peers of array [...].

Warning: None of the arrays has valid efficiency peers.

Warning: Aborting efficiency calculations (will limit the report's output).
````
...
````
Warning: An inter-array efficiency has already been calculated for array [...].
