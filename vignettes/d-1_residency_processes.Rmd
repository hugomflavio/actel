...
Based on the table above, actel will proceed to calculate the amount of time spent per day (or hour, if `timestep = "hours"`) at each location (in seconds). This leads to a very long table displaying both the number of seconds spent per day/hour at each location, as well as the respective percentages. It also contains an additional column showing the location where the fish spent the most time each day/hour. [You can find an example here](d-2_residency_results.html#time.ratios).


## Global ratios

Using the table above as a reference for each fish, actel is then able to calculate how many of your fish were at each location during each passing day/hour. Actel calculates two similar tables, one with absolute numbers, and one with percentages. [You can find an example here](d-2_residency_results.html#global.ratios).
