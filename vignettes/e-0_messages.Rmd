...
vignette: >
  %\VignetteIndexEntry{5) Errors and messages}
...
Note :
  : As actel evolves, some new errors and messages may accidentally be left out of this page. If you encounter a message or error that is not reported here, please open an issue at [actel's GitHub repository.](https://github.com/hugomflavio/actel/issues)
...
1. **Something is wrong in the input files**
	...
	Earlier versions of actel used a 'Group' column to group arrays. This fail-safe allows you to bypass the issues this would cause.
	...
	Not all files in your 'detections' folder were recognised as receiver logs. Make sure you did not edit the detection files manually before feeding them to Actel. If the problem persists, contact the developer team.
...
1. **Something is off with the receivers**
  ...
  W: Detections from receivers [serial numbers] are present in the data, but these receivers are not part of the study's stations. Doublecheck potential errors.
	...
	Somewhere in your detection files, there are detections for receivers that are not listed in the spatial file. If some of your target tags are detected in these receivers, this warning will escalate to the one below.
  ...
  Which option should be followed?(a/b) 
	...
	At least one of your target tags were detected in a receiver that is not listed in the spatial file. If you are truly unsure about the provenance of this unknown receiver, you can temporarily include it to the stations list, and then have a look at the movement events to find out when were your fish detected there. Note that this receiver will not be used to determine the fish's arrival/departure points from the various sections.

	```
  W: No detections were found for receiver(s) [receivers serial].
	```

	One or more of the receiver listed in the spatial file had 0 detections in the detection files (not even noise/stray detections). While this is not bad on its own (perhaps nothing passed through that receiver), it may also mean that there are some detection files missing from the 'detections' folder.
...
1. **Array mismatch**
	...
	There could be a typo in the array listed as first array for a release site. Doublecheck the spatial file and make sure the arrays listed in the release sites match the arrays listed in the receivers.
...
1. **Unknown receivers**
	...
	If you are running the analysis with unknown receivers (see the messages for these above), these will be excluded from the timetable analysis. If the fish only moved through unknown receivers, the message will escalate to a warning.
...
1. **Pre-existing files**
	...
	M: An actel report is already present in the present directory, saving new report as 'actel_report.#.html'.	
	```
