...
The R package RSP ([available here](https://github.com/YuriNiella/RSP#rsp)) allows you to run spatial analyses on the habitat use of your fish using the output of your actel analysis. With the RSP toolkit you can analyse the fine scale movements of aquatic animals while accounting for the surrounding land masses. The animal movements between detections are recreated to have occurred exclusively in water and the utilization distribution areas are limited by the land contours, providing realistic estimations of space use. 
...
Sometimes it can be useful to compare the movements of multiple tags at the same time. Comparing individual detection plots works, but beign able to overlap plots would be much nicer! That's exactly why I created the `plotMoves()` function. This function allows you to overlap the movements of multiple tags.
...
A classic plot in migration analyses is one showing the cumulative arrival of fish at a specific array. The function `plotArray()` allows you to obtain both the simultaneous presence at an array (if `cumulative = FALSE`, the default), or the cumulative arrivals at an array (yes, you guessed it, with `cumulative = TRUE`). 
...
# plotting lines or bars depending on wether the plot is cumulative
