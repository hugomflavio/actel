...
The initial processes of migration() are the same as those from [the explore() function](b-1_explore_processes.html). However, when it comes to movement validity, actel will now also check if the fish was detected behind its release site. Should this happen, the following warning will be issued:

````
W: Fish R64K-1234 was detected behind its release site! Opening relevant data for inspection.
   Release site: Trap
...
By default, actel will consider the very last event of each section as the point when the fish left that section. For this example, that means that the fish would be considered to have left the river (for the last time) on event **5**, entered the fjord (for the last time) in event **6** and finally left the fjord in event **7**.
...
If actel were to keep the default rules (i.e. the fish left the river on event **7**, and left the fjord on event **6**), then it would be impossible to find a suitable first Fjord event. There are multiple options you can follow here, e.g.:

1. Discard the Fjord detections (i.e. events 3, 4, 5 and 6)

2. Assume the fish was predated between event 4 and 5, and discard events 5, 6 and 7.
...
You can then find all your comments in the [html report](c-2_migration_results.html#actel_migration_report.html-if-report-true) and in the [final data frame](c-2_migration_results.html#status.df).

## Combining the time data with the biometrics

The hardest part is done. Now, actel will automatically combine the time information with the information you included in the biometrics file. This new data frame is called *status.df*, and will be available to you in the function output. It is also here that the [success.arrays variable](c-0_migration.html#success.arrays) comes in. If your fish was last detected on an one of those arrays, it will be assigned as "Succeeded".

## Inspecting the results

Finally, actel will compile some summary tables, prepare the report, and finish up. It is time to [have a look at the results!](c-2_migration_results.html)
