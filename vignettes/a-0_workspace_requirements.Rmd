...
Release.date
  ...
  : Note that the timestamps must be in the **local time zone** of the study area, which you will later supply in the `tz.study.area` parameter.

Signal 
  : Corresponds to the code emitted by the tag that was implanted on the fish. If you are unsure as to what signal is being emitted by your tag, you should ask the tag manufacturer more about the differences between code spaces and signals.
...
Note:
  : You can then append as many columns as you want to **biometrics.csv**. Importantly, if the biometrics have columns with keywords such as *length*, *weight* or *mass*, graphics showing the distribution of these parameters per fish group will be drawn for you in the report.
