...
test_that("plotDetections is working", {
	p <- plotDetections(example.results, tag = "R64K-4457")
	expect_that(p, is_a("ggplot"))
})

test_that("array aliases are running fine", {
	p <- plotDetections(example.results, tag = "R64K-4457", array.alias = c(River0 = "test"))
	expect_that(p, is_a("ggplot"))
	expect_warning(plotDetections(example.results, tag = "R64K-4457", array.alias = c(wrong_name = "test")),
		"Could not find array wrong_name in the study's arrays.", fixed = TRUE)
})

test_that("personalised colours are running fine", {
	p <- plotDetections(example.results, tag = "R64K-4457", col = rev(gg_colour_hue(10)))
	expect_that(p, is_a("ggplot"))
	expect_warning(plotDetections(example.results, tag = "R64K-4457", col = "red"),
...
test_that("run code for colour frames", {
	...
	p <- plotDetections(xresults, tag = "R64K-4457")
	expect_that(p, is_a("ggplot"))
