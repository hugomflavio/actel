...
test_that("explore stops when any argument does not make sense", {
  ...
  if (length(suppressWarnings(packageDescription("gWidgets2tcltk"))) == 1) {
      ...
      paste0("GUI is set to 'needed' but package 'gWidgets2tcltk' is not available. Please install it if you intend to run GUI. Disabling GUI (i.e. GUI = 'never') for the current run."),
      fixed = TRUE)
  }
