test_that("loadBio stops if arguments or file are missing", {
	# Missing arguments
	expect_error(actel::loadBio(), "'file' is missing.", fixed = TRUE)
	expect_error(actel::loadBio(file = "test"), "'tz' is missing.", fixed = TRUE)

	# Missing file
	expect_error(actel::loadBio("biometrics.csv", tz = "Europe/Copenhagen"))
})

test_that("loadBio stops if there are duplicated columns", {
	# Duplicated cols
	bio <- actel::example.biometrics
...
test_that("loadBio fails if needed columns are missing", {
	...
	expect_message(actel::loadBio("biometrics.csv", tz = "Europe/Copenhagen"), 
...
test_that("loadBio stops if column content is unexpected", {
	...
	write.csv(bio, "biometrics.csv", row.names = FALSE)
	expect_warning(actel::loadBio("biometrics.csv", tz = "Europe/Copenhagen"), 
	...
	write.csv(bio, "biometrics.csv", row.names = FALSE)
	expect_warning(output <- actel::loadBio("biometrics.csv", tz = "Europe/Copenhagen"), 
	...
	write.csv(bio, "biometrics.csv", row.names = FALSE)
	expect_message(output <- actel::loadBio("biometrics.csv", tz = "Europe/Copenhagen"), 
...
test_that("loadBio output matches actel::example.biometrics", {
	# Output is correct
	write.csv(actel::example.biometrics, "biometrics.csv", row.names = FALSE)
	bio <- actel::loadBio("biometrics.csv", tz = "Europe/Copenhagen")
	expect_equal(bio, actel::example.biometrics)
	file.remove("biometrics.csv")
})
