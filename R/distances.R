...
#' the distance. Ignored if actel = TRUE (default), as the 'spatial.csv' is 
#' loaded as starters.
#' @param targets A data frame with the points to which a way must be found. 
#' Ignored if actel = TRUE (default), as the 'spatial.csv' is loaded as targets.
...
#' @param file_path Specify the file path to the 'spatial.csv' file if not in 
...
distancesMatrix <- function(t.layer, starters = NULL, targets = starters,
  coord.x = "x", coord.y = "y", id.col = NULL, file_path = NULL, actel = TRUE){
  # initial checks on package presence
  aux <- c(
    length(suppressWarnings(packageDescription("raster"))),
    length(suppressWarnings(packageDescription("gdistance"))),
  ...
  if (actel) {
    ...
    if (!is.null(starters) | !is.null(targets)) {
      warning("starters' or 'targets' were set but will be ignored because", 
              " 'actel' is set to TRUE. Set 'actel' to FALSE to use the", 
              " 'starters' and 'targets' arguments.", 
              call. = FALSE, immediate. = TRUE) 
    }
   # Added new check for 'file_path' being specified in function 
    if(!is.null(file_path)) {
      
      # If a 'file path' is specified, the complete file to the 'spatial.csv' is 
      # constructed
      new_path <- paste0(file_path,"spatial.csv")
      
      # Added check in case 'file_path' did not lead to the 'spatial.csv' with 
      # stop event
      ...
      starters <- targets <- loadSpatial(input = new_path) 
    } else {
      
      # If no file path is specified, then 'spatial.csv' is assumed to be in 
      # the main working directory
  ...
  if (interactive() & actel) { # nocov start
    ...
    if (decision == "y") {
      ...
      if(!is.null(file_path)) {
        ...
        new_path <- paste0(file_path,"distances.csv")
