...
# print html report
  if (report) {
    ...
    rmarkdown::render(reportname <- mbPrintMigrationRmd(name.fragment = name.fragment, header.fragment = header.fragment, 
...
#' @keywords internal
#' 
mbPrintMigrationRmd <- function(name.fragment, header.fragment, biometric.fragment, efficiency.fragment, array.overview.fragment,
  survival.graph.size, individual.plots, circular.plots, spatial, deployments){
  appendTo("Screen", "M: Producing final report.")
  if (file.exists(reportname <- paste("Report/actel_migration_report", name.fragment, ".Rmd", sep = ""))) {
    continue <- TRUE
